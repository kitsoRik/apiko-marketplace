	...
	fields: () => ({
		savedProducts: {
			...
			resolve: async (
				...
				return getProductsByIds(savedIds)
					.skip((page - 1) * limit)
					.limit(limit);
			},
		},
		...
		locations: {
			...
			resolve: async (
				source,
				{ namePattern, page = 1, limit = 10 }
			) => {
				return await getLocationsByNamePattern(namePattern)
					.skip((page - 1) * limit)
					.limit(limit);
			},
		},
	...
	fields: () => ({
		changeSavedStateOfProduct: {
			...
			resolve: (source, { id, state }, { user }) => {
				if (!user) throw "WHAT";
				if (state && user.savedProducts.find((p) => p === id))
					throw "WHAT1";
