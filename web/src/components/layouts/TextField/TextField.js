...
const TextField = ({
	...
	const onKeyDown = (e) => {
		switch (e.key) {
			...
			case "Enter": {
				if (!autoCompleteOptions && !autoCompleteOptionsWhenEmpty)
					return;
				e.preventDefault();
				let _value;
				if (value === "") {
	...
	const autoCompleteTruthOptions = (autoCompleteOptions ?? []).filter(
		({ props: { textValue } }) =>
			new RegExp(value).test(textValue) && value !== textValue
	);
	...
	return (
		<div
			...
			onBlur={_.debounce(() => setInFocus(false), 100)}
			tabIndex="1"
