...
const Register = ({ history }) => {
	...
	const validate = ({ email, password, passwordAgain, fullName }) => {
		...
		else if (!/[a-zA-Z\d+]+@[a-zA-Z]+\.[a-zA-Z]{1,5}/.test(email))
			errors.email = "Email is not valid";

		if (password.length < 8)
			errors.password = "Length must be greater or equal eight";
		if (passwordAgain.length < 8)
			errors.passwordAgain = "Length must be greater or equal eight";
		else if (passwordAgain !== password)
			errors.passwordAgain = "Passwords is not equal";

		if (fullName.length < 2)
			errors.fullName = "Length must be greater or equal two";
