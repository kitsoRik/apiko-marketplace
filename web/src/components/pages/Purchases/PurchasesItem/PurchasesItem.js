...
const PurchasesItem = ({
	...
	const { product, seller, statuses } = data.purchase;
	...
	const visibleCloseButton =
		isShopper && !isClosed && !!isPosted && !isCanceled;
	...
	return (
		<div className="purchases-item-page">
			<div className="purchases-item-page-product">
				...
				<span className="purchases-item-page-product-title">
					{product.title}
				</span>
			</div>
			<div className="purchase-item-page-statuses">
				{statuses.map((s) => (
					<div
						className="purchases-item-page-statuses-item"
						status={s.status}
					>
						<span>{s.status}</span>
