...
const HeaderSearchPanel = ({
	...
	const onSearchProductsHint = useCallback(
		_.debounce(() => searchProductsHint(), 700),
		[]
	);
	...
	return (
		<div className="home-page-header-panel">
			<TextField
				...
				value={title}
				loading={loading}
				autoCompleteOptions={data?.products.map((product) => (
				...
				autoCompleteOptionsWhenEmpty={getLatestProductsTitleQuery().map(
					(item) => (
						<TextFieldAutocompleteOption
							className="home-page-header-panel-text-field-autocomplete-recent-products-item"
							key={item}
							textValue={item}
							value={item}
							icon={
			...
			<LocationTextField
				onLocationIdChange={(locationId) =>
					changeProductsSearchQuery({ locationId })
				}
...
export default connect(
	({
		products: {
			searchQuery: { title, locationId },
			searchProductsHintQuery,
		},
	}) => ({ title, locationId, searchProductsHintQuery }),
