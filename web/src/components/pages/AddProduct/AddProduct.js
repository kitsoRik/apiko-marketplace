...
const AddProduct = ({ history }) => {
	...
	const validate = ({
		...
		if (price === "") errors.price = "Price must be";
			...
			errors.price = "Price must contains only number";
		else if (!/^\d+(\.\d{1,2})?\$?$/.test(price))
			errors.price = "Unvalid value after point";
		...
		if (photos.length > 5) {
			if (!photosWarned) {
				...
				notifyError(
					`Max numbers of photos is 5, but you gave ${photos.length}`
				);
				setFieldValue(
					"photos",
					photos.filter((p, i) => i < 5)
				);
			}
		}
	...
	const setMultiplePhotos = (photos) =>
		setFieldValue("photos", values.photos.concat(photos));
