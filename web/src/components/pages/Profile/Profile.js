...
import {
	USER_PROFILE_DATA_QUERY,
	USER_QUERY,
} from "../../../apollo/queries/user-queries";
			...
			<Panel className="profile-page-panel">
				...
				{!loading && (
					<div className="profile-page-main-content">
						{tabIndex === 0 &&
							!!userProfileDataQuery.data?.user
								?.productsCount && (
								<FeedbacksContent
									userId={!id ? currentUser.id : id}
								/>
							)}
						{false && (
							<SalesContent userId={currentUser.id} />
						)}
						{tabIndex === 2 &&
							!!userProfileDataQuery.data?.user
								?.productsCount && (
