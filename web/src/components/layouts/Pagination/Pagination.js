...
const Pagination = ({
	...
	const middlePages = _.times(
		MAX_PAGE_SEQUENCE + 2,
		(p) => page + p - 2
	).filter((p) => p > 0 && p <= pages);

	const leftPages = _.times(MAX_PAGE_SEQUENCE, (p) => p + 1).filter(
		(p, i) =>
			i < 5 &&
			p > 0 &&
			p <= pages &&
			!middlePages.find((m) => m < p || m === p)
	);
	const rightPages = _.times(
		MAX_PAGE_SEQUENCE,
		(p) => pages - 5 + p + 3
	).filter(
		(p, i) =>
	...
	return (
		<Form className={`pagination ${className ?? ""}`}>
			<PaginationSector
				disabled={page === 1}
				className="pagination-sector-prev"
				value="<"
			...
			<PaginationSector
				disabled={page >= pages}
				className="pagination-sector-next"
				value=">"
				onClick={() => changePage(page + 1)}
			/>
		</Form>
