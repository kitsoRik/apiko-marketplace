...
exports.getProductsIdsByUserId = (id) =>
	userModel
		.findOne({ id }, { productsIds: true })
		.then(({ productsIds }) => productsIds);
exports.getSavedProductsIdsByUserId = (id) =>
	userModel
		.findOne({ id }, { savedProducts: true })
		.then(({ savedProducts }) => savedProducts);

exports.getUserByProductId = (productId) =>
	userModel.findOne({ productsIds: { $elemMatch: { $eq: productId } } });
...
exports.saveUserById = (id, fullName, locationId, phone, iconName) =>
...
exports.updateUserIcon = (id, iconName) =>
	userModel.findOneAndUpdate({ id }, { iconName }, { new: true });

exports.addProductByUserId = async (userId, productId) =>
	userModel.findOneAndUpdate(
		{ id: userId },
		{
			productsIds: [
				...(await this.getProductsIdsByUserId(userId)),
				productId,
			],
		}
	);
...
exports.addProductToCardByUserId = async (id, productId, count) => {
	...
	if (cp)
		return this.changeCartItemCountByUserId(
			id,
			productId,
			cp.count + count
		);
	return userModel.findOneAndUpdate(
		{ id },
		{ $push: { cartProducts: { productId, count } } },
		{ new: true }
	);
};

exports.clearCartByUserId = (id) =>
	userModel.findOneAndUpdate({ id }, { cartProducts: [] }, { new: true });
